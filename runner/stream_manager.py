...
async def socket_listener(symbol):
    # 🔥 ИЗМЕНЕНИЕ: Базовый таймфрейм теперь 15 минут
    interval = "15m" 
    stream = f"{symbol.lower()}@kline_{interval}"
    url = f"{BINANCE_WS}/{stream}"

    # Загрузка истории (грузим 15-минутки)
    history = fetch_initial_history(symbol, interval)
    market_data[symbol] = history

    while True:
        try:
            print(f"🎧 Подключение к потоку {symbol} ({interval})...")
            async with websockets.connect(url, ping_interval=20, ping_timeout=20) as ws:
                print(f"✅ Поток {symbol} активен")
                while True:
                    msg = await ws.recv()
                    ...
                    if kline["x"]:  # Свеча закрыта
                        ...
                        # Храним 500 свечей по 15м (этого хватит на неделю истории)
                        MAX_CANDLES = 500
                        if len(current_list) > MAX_CANDLES:
                            current_list.pop(0)
...
async def start_stream_manager():
    tasks = []
    print(f"🚀 Запуск Intraday-потоков для {len(WATCHLIST)} монет...")
    for coin in WATCHLIST:
        tasks.append(socket_listener(coin))
